...
SCAN_TOPIC = "/experiment/critical/scan"
POSITION_TOPIC = "/experiment/critical/local_position"
...
VFH_ENTER_AVOID_DISTANCE = 5.0

# AVOID -> RETURN_PATH 전환 기준
MIN_AVOID_TIME = 1.5
CLEAR_FRONT_DISTANCE = 7.0
CLEAR_SIDE_DISTANCE = 1.5
CLEAR_COUNT_THRESHOLD = 5

# RETURN_PATH 중 다시 AVOID로 들어가는 기준
RETURN_REENTER_DISTANCE = 3.0
RETURN_REENTER_SIDE_DISTANCE = 1.2
...
class VFHPlusConfig:
    ...
    # 5.0m 이내 장애물만 VFH+ histogram에서 위험 영역으로 반영
    active_distance: float = 5.0
...
class VFHPlusDroneControl(Node):
    ...
    def scan_callback(self, msg):
        ...
        self.left_avg = sum(left) / len(left) if left else self.vfh_config.max_range
        self.right_avg = sum(right) / len(right) if right else self.vfh_config.max_range
    ...
    stable_heading_for_log,
        ...
        data = {
            ...
            "vfh_state": self.vfh_planner_state,
            "raw_heading": math.degrees(self.vfh_selected_heading),
            "smooth_heading": math.degrees(stable_heading_for_log),
            "vx": vx,
            "vy": vy,
            "vz": vz,
            "target_x": target_x,
            "target_y": target_y,
            "takeoff_stable": int(self.takeoff_stable),
            "avoid_enabled": int(self.avoidance_enabled),
            "avoid_t": self.avoid_elapsed_for_log,
            "avoid_prog": self.avoid_progress_for_log,
            "clear_cnt": self.front_clear_count,
        }
